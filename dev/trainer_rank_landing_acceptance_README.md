...
| dense attention classes (`dev/trainer_rank_cost_calibration_lattice.sky.yaml`: Qwen3-1.7B, Qwen3-8B, Qwen3-14B; Qwen3-4B real data at CP4/TP2×CP2) | 4x H200 | same protocol on the Qwen3-tokenized Ellavox groups and synthetic families at TP1 × CP1/2/4, TP2 × CP1, TP2 × CP2 (70 cells each); candidate rows record the CP plan structure (`cp_plan`: wave count, max rank tokens, summary cost) and the version-1 score | `dense-attn-h2048`/`h4096` at CP1, CP2, TP2 (42 cells; pairwise 96.5% / 99.9%, max regret 1.0% / 1.1%), `dense-attn-h5120` also at TP2 × CP2 (56 cells; 99.9%, max 1.0%); TP1 × CP4 through the two-stage re-ranker where it pays back against both the cheap and the version-1 selection on all and held-out cells: Qwen3-8B (max 0.8%; planning 1.6% vs 1.8% saved) and Qwen3-14B (4.1%; 1.0% vs 1.2%); on Qwen3-1.7B it ranks (2.6%) but costs 2.6% against 2.0% saved, so CP4 keeps version 1; TP2 × CP2 not admitted for the two smaller classes; the dense table withholds Qwen3-4B × TP1 × CP4 (35% vs version 1's 15%) |
| Qwen3-30B-A3B class (`dev/trainer_rank_cost_calibration_lattice.sky.yaml`, EP1 at CP1/2/4/TP2, EP2 at CP2/CP4/TP2, EP4 at CP4; CP2 and CP4 EP1/EP4 re-measured with sixteen rounds) | 4x H200 | 112 cells on the Qwen3-tokenized Ellavox groups and synthetic families; 3 CP1 cells fail single-GPU memory admission (#848); sporadic multi-second stalls on ~4% of rounds, medians hold at sixteen rounds | `attn-moe-h2048-h200-bf16`: direct at CP1, CP2, CP2 × EP2, TP2, TP2 × EP2 (67 cells; 96.3%, max 2.2%); CP4 (every EP) measured but keeps version 1: the re-ranker ranks it (max 2.1%) but saves 0.9% against a 1.1% planning cost and is 0.3% worse than version 1 on held-out cells |
| Qwen3.5-27B class (`dev/trainer_rank_cost_calibration_lattice.sky.yaml`, TP1 × CP1/2/4, TP2 × CP1, TP2 × CP2 in two halves) | 4x H200 | 70 cells on the Qwen3.5-tokenized Ellavox groups and synthetic families; 4 cells incomplete under single-GPU memory admission (#848), 1 lost to an NCCL timeout | `dense-gdn-h5120-h200-bf16`: admitted at CP1, CP2, CP4, TP2 (52 cells; pairwise 97.0%, max regret 3.4%, held-out 1.8%) without a re-ranker; TP2 × CP2 fails its gates (p95 5.1%) and keeps version 1, which lost up to 112% at CP4 on this class |
| `dev/trainer_rank_cost_calibration_certificate_<table>.json` (one per calibrated table, compact: one line per cell) + `tests/unit/test_planner_cost_certificate.py` | CPU | the shipped table is the certified table (hash) and the certified metrics hold on the recorded per-cell aggregates; exact cell identities match each table's manifest (dense: the 58 fitted cells plus the 16 excluded blind-spot cells; attention classes: TP2 × CP2 excluded with reasons on the two smaller classes; TP1 × CP4 fitted directly since the planner recalibration retired the re-rankers) and each table's withheld pairs are exactly the manifest's; tables with a re-ranker (none shipped since 2026-09-08): the shipped re-ranker equals the certificate's `reranker` block and the two-stage selection recomputed from the recorded aggregates reproduces its metrics and passes the gates on every re-ranked shape; the admitted execution classes (device classes, dtypes, model geometries, parallel shapes) equal the certified cells' and every geometry was measured at every admitted shape; runners fail loudly on any cell failure and the fitter refuses incomplete evidence unless exclusions are explicit (the two Ellavox CP4 cells that hung in the campaign were a harness desynchronization, issue #840, fixed and re-measured) | pass |
| grad mode in layout selection (`_PlannerFacts.grad_enabled`) | CPU (megatron env) | groups that run without gradients keep the version-1 score and their own cache identity: the tables and re-rankers were calibrated on forward + backward | `tests/unit/test_trainer_rank_validation.py` |
| `tests/unit/test_planner_rerank.py` | CPU | two-stage selection: shortlist (k + depth-one anchor), lowest second-stage score wins, ties keep the cheaper layout, tables validate re-ranked shapes against their re-ranker, `select_scoring` returns the re-ranker on re-ranked shapes only | pass |
| `tests/unit/test_context_parallel_plan_evaluation.py` | CPU (megatron env) | the vectorized CP assignment-search evaluator equals the scalar reference on randomized instances (incl. the long-row path) and the search result is unchanged | pass |
| `tests/unit/test_planner_cost_profile.py` | CPU | a fitted table applies only to the execution classes it was measured on (device class, dtype, Megatron-config model geometry, TP × CP × EP × ETP shape), bound to its certificate by test; outside every table the version-1 score is used verbatim, unmeasured shapes such as CP8 and withheld pairs (Qwen3-4B at TP1 × CP4) included | pass |
